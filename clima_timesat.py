...
def calcular_graus_dia(TM, Tm, TB=38, Tb=9):
    # Zero if TM exceeds TB (upper base) — excess heat condition
    if TM > TB or Tm < Tb:
        GD = 0
    # TB > TM > Tm > Tb: both daily temperatures are fully inside the base limits
    elif TB > TM > Tm > Tb:
        GD = (TM - Tm) / 2 + Tm - Tb
    # TB > TM > Tb > Tm: upper base falls between Tm and TM
    elif TB > TM > Tb > Tm:
        GD = ((TM - Tb)**2) / (2 * (TM - Tm))
    # TB > Tb > TM > Tm: both daily temperatures are below the upper base
    elif TB > Tb > TM > Tm:
        GD = 0
    # TM > TB > Tm > Tb: lower base falls between Tm and TM
    elif TM > TB > Tm > Tb:
        GD = (2*(TM-Tm)*(Tm-Tb) + (TM-Tm)**2 - (TM-TB)**2) / (2*(TM-Tm))
    # TM > TB > Tb > Tm: both base limits fall between Tm and TM
    elif TM > TB > Tb > Tm:
        GD = 0.5 * ((TM-Tb)**2 - (TM-TB)**2) / (TM-Tm)
    else:
        # any other case not explicitly covered
        GD = 0
    return GD


# remove day 366 from leap years to ensure consistent DOY across years
...
#----------------------------------------
# Climate anomaly and mean calculations
#----------------------------------------
# Temperature is accumulated from planting through pre-harvest.
# Precipitation is evaluated in three windows:
#   - establishment drought: data_ini → mos_dt1
#   - harvest rain:          mos_dt2 → data_fim
#   - full season:           data_ini → data_fim
# SPI covers the entire growing season.
def process_climate_safra(row, geometry, tmed_ds, clim_tmed_ds, tmax_ds,
    ...
    #
    # lat/lon for spatial masking (ROI)
    lon = tmed_clip.lon.values
    lat = tmed_clip.lat.values
    xx, yy = np.meshgrid(lon, lat)
    #
    # compute GDD for the area
    ...
    # build a NaN mask on the precipitation grid
...
# same methodology for precipitation
...
if os.path.exists(CSV_OUT):
    ...
    df = df[(df['ERROR'] == 0)]
    if len(df) <= 3:
        print('too few seasons')
        quit()

    prod = gdf.loc[mun].dropna()
    if len(prod) < len(df):
        print('fewer IBGE seasons than TIMESAT seasons')
        quit()

    print(f'generating output for {mun}')
    print(f'TIMESAT seasons: {len(df)}')
    print(f'IBGE seasons   : {len(prod)}')
